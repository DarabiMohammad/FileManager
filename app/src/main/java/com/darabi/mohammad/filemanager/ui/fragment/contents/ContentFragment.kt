...
class ContentFragment @Inject constructor (
    ...
    private val copyMoveDialog: CopyMoveStatusDialog,
        ...
        mainViewModel.onPathSelected.observe(viewLifecycleOwner, {
            it?.let { singleEventWrapper ->
                singleEventWrapper.getContentIfNotHandled()?.let { destinationData ->
                    //checking if targetPath is subfolder of selected files
                    if (contentViewModel.isSubFolderOFSelectedItems(destinationData.first))
                        makeToast(getString(R.string.parent_to_Child_copy_error))
                    else
                        copyMoveDialog.forAction(destinationData.first, destinationData.second).show(childFragmentManager)
                }
            }
        })
    }
